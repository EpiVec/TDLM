...
test_that("check messages and errors", {
  ...
  expect_message(check_format_names(vectors=vectors, matrices=matrices, check="format_and_names"), 
                "The inputs passed the format_and_names checks successfully!")
  
  expect_message(check_format_names(vectors=vectors, matrices=matrices, check="format"), 
                 "The inputs passed the format checks successfully!")
  
  expect_error(check_format_names(vectors=NULL, matrices=NULL, check="format"), 
               "At least one of the vectors or matrices argument should be non-null.")
  
  expect_error(check_format_names(vectors=vectors, matrices=NULL, check="test"), 
