...
test_that("class data.frame and values", {
  ...
  res <- run_law_model(law = "GravExp", mass_origin = mi, mass_destination = mj, distance = distance, opportunity = NULL, param = 0.01,
                       model = "DCM", nb_trips = NULL, out_trips = Oi, in_trips = Dj, average = FALSE, nbrep = 1, maxiter = 50000, mindiff = 0.00001,
                       write_proba = TRUE,
                       check_names = TRUE)
  
  m=c("CPC", "NRMSE", "KL", "CPL","CPC_d", "KS")
  val <- gof(sim=res, obs=odobs,  measures = m, distance = dist, bin_size = 2,
             use_proba = FALSE,
             check_names = TRUE)     
...
test_that("check errors", {
  ...
  m=c("CPC", "NRMSE", "KL", "CPL","CPC_d", "KS")
  expect_error(gof(sim=res, obs=odobs,  measures = m, distance = dist, bin_size = 2,
                   use_proba = TRUE,
                   check_names = TRUE) , 
