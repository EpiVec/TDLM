...
#' demand).
#' 
#' @param mass_destination a numeric vector representing the mass at destination
#'  (i.e. attractiveness).
#' 
...
#' 
#' @param nb_trips an integers indicating the total number of trips.
#' 
...
#' 8) Uniform law (`law = "Rand"`). The argument `mass_origin` will be used to
...
#'
#' @seealso [check_format_names()] [extract_opportunities()]
#'
#' @examples
#' data(mass)
#' data(distance)
...
run_law_model <- function(law="NGravExp", 
  ...
  # Controls LAW
  laws = c("GravExp","NGravExp","GravPow","NGravPow","Schneider","Rad","ExtRad",
           "Rand")
...
GravExp, NGravEx, GravPow, NGravPow, Schneider, Rad, ExtRad or Rand", 
         call. = FALSE)
  }
  if((law %in% dist_laws) | (law %in% oppo_laws)){
    controls(
      args = NULL,
      vectors = list(mass_origin = mass_origin, 
...
UM, PCM, ACM or DCM", 
  ...
  if(model == "UM"){
    controls(args = nb_trips, type = "positive_integer")
  }
  if(model == "PCM"){
    controls(
      args = NULL,
  ...
  if(model == "UM"){
    mass <- cbind(mass_origin, mass_destination, rep(1,nbloc),rep(1,nbloc))
  }
  if(model == "PCM"){
  ...
  args <- paste0(wdin, " ", wdout, " ", law, " ", beta, " ", pij_only, " ", 
  ...
  outputs$args = data.frame(Args = c("law", "param", "model", "nbrep"),
                            Value = c(law, param, model, nbrep))
