...
[sf](https://cran.rstudio.com/web/packages/sf/index.html) 
...
This is just an estimation so in order to rigorously calibrate and evaluate the
...
```{r}
cpc <- aggregate(calib$CPC, list(calib$Parameter), mean)[,2]

plot(seq(0.05,0.1,0.005), cpc)
```
